...
export const getMentorshipData = async (): Promise<MentorshipEntry[]> => {
  ...
  if (!csvUrl) {
    console.warn('VITE_PUBLISHED_CSV_URL not found in environment variables');
    return [];
  }
...
const parseCSV = (csvText: string): MentorshipEntry[] => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!feedback) {
      console.log('Skipping row - no feedback');
      continue;
    }
    ...
    console.log('Created entry:', {
      name: entry.name,
      topic: entry.topic,
      hasLinkedIn: !!entry.linkedin,
      hasGitHub: !!entry.github,
      feedbackLength: entry.feedback.length,
    });

    data.push(entry);
  }

  console.log(`\n✅ Successfully parsed ${data.length} entries`);
  return data;
};
